...
# carl.py - combined chess_engine and chess_ai into one text.
# currently working on single player code
...
from numpy import array, average  # for move generation vector arithmetic
...
def opposite(col):  # i know, this is a weird function
    if col == 'white':
        return 'black'
    elif col == 'black':
        return 'white'
...
class Piece(object):
    ...
    def move(self, newpos, board_obj, force=False):
        ...
        if self.is_pseudolegal(newpos, board_obj) or force:
            ...
            print "ILLEGAL MOVE %s to %s" % (parse(self.pos), parse(newpos))
            print board_obj
            print board_obj.movehistory
        ...
        for move in moveset:  # check for straightforward moves
            if not isinstance(board_obj.squares[tuple(move)], Piece):  # valid move
                if not attack_map:
                    ...
                    if self.pos[1] == self.homerow:
                        doublemove = array(self.pos) + self.movedir*2
                        if not isinstance(board_obj.squares[tuple(doublemove)], Piece):
                            possible_moves.append(tuple(doublemove))
                            possible_specials.append([tuple(doublemove), 'pawndouble'])
                    if promotable:
...
class Board(object):
    def newgame(self, ai_plays = None):
...
#                      'b_g_pawn', 'b_h_pawn']
        ...
        if self.whose_turn == self.ai_plays:
            make_best_move(self)
        elif self.ai_plays == 'both':
            while True:
                make_best_move(self)



    def gen_all_moves(self, col, attack_map=False):
  ...
  +-------------------------+\n""" % files
    ...
    def advance_turn(self):  # this is the refresh/update function
        ...
        log("Setting en passant availability")
        if self.movehistory[-1][1] == 'pawndouble':
            lastmove = self.movehistory[-1][0]
            en_passant_pos = (lastmove[0][0], int(average([lastmove[0][1], lastmove[1][1]])))
            self.enpassant = en_passant_pos
        else:
            self.enpassant = None
...
#            self.enpassant = None
    ...
    def rewind(self):  # opposite of advance turn
        ...
        if len(self.movehistory) > 0:  ### replace this try statement with just checking if movehistory exists
            if self.movehistory[-1][1] == 'pawndouble':
                lastlastmove = self.movehistory[-1][0]
                en_passant_pos = (lastlastmove[0][0], int(average([lastlastmove[0][1], lastlastmove[1][1]])))
                self.enpassant = en_passant_pos
            else:
                self.enpassant = None
...
# piece square tables and piece values from a simplified evaluation function by Tomasz Michniewski
...
def negamax(board, max_ply=2, ply=2, lastmove=None, bestmove=None):
    ...
    elif board.winner == col:
        return (100000000, lastmove)
    elif board.winner == opposite(col):
        return (-100000000, lastmove)
    elif board.winner == 'nobody':
        return (0, lastmove)
...
#        print "trying %s to %s" % (parse(lastmove[0]), parse(lastmove[1]))
    ...
    for move in movelist:
        ...
        try:
            ...
            print board
            print "The move just gone was:"
            print movelist[movenum-1]
            sys.exit()
...
def play_ai_vs_ai():
    mainboard = Board()
    mainboard.newgame(ai_plays='both')

def play_sp(human_col = 'white'):
    mainboard = Board()
    mainboard.newgame(ai_plays=opposite(human_col))
...
def gui_process(jsonstring):
    ...
    if command == u'getMoves':
        ...
        if mainboard.whose_turn == mainboard.ai_plays:
            sys.stderr.write("it is %s's turn and the computer is %s, so computer is making a turn" % (mainboard.whose_turn, mainboard.ai_plays))
            make_best_move(mainboard)
            gui_sendboard(mainboard)
    elif command == u'newGame':
        mainboard = Board()
        if jsondict['humanCol'] != 'both':
            mainboard.newgame(ai_plays=opposite(jsondict['humanCol']))
        else:
            mainboard.newgame()
        gui_sendboard(mainboard)
